...
Nginx serves the pre-built React bundle and forwards all `/api/*` requests to Uvicorn on `127.0.0.1:8000`. Both processes are managed by `supervisord`. `start.sh` launches Uvicorn; FastAPI lifespan is the sole automatic migration owner and runs `alembic upgrade head` before the app serves requests. SQLite startup assumes one app process migrates a given database at a time, so direct Alembic commands and parallel app startups must not overlap.
